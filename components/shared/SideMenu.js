...
const SideMenu = ({ visible, onClose }) => {
  ...
  const menuWidth = width * 0.7 // Menu takes up 70% of screen width
  ...
  const renderIcon = (item) => {
    if (item.type === "feather") {
      return <Feather name={item.icon} size={24} color={theme.colors.primary} />
    }
    return <Ionicons name={item.icon} size={24} color={theme.colors.primary} />
  }

  if (!isRendered) return null

  // Use hardcoded solid colors as a fallback
  const backgroundColor = theme.dark ? "#0a0a1a" : "#ffffff"
        ...
        <Animated.View
          style={[
            styles.drawer,
            {
              ...
              backgroundColor: backgroundColor, // Hardcoded solid color
              borderRightWidth: 1,
          ...
          <SafeAreaView style={styles.safeArea}>
            <View style={[styles.header, { borderBottomColor: theme.colors.border }]}>
              ...
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Ionicons name="close-outline" size={28} color={theme.colors.buttonText} />
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  drawer: {
    ...
    top: 0,
    bottom: 0,
    zIndex: 10, // Ensure drawer is above overlay
  },
  dismissArea: {
    height: "100%",
    position: "absolute",
